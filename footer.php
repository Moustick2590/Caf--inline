<!-- site-content -->
<footer class="container-fluid">
    <div id="footer_container" class="container">
        <nav class="navbar">
            <a class="navbar-brand" target="_blank" href="https://www.accesscodeschool.fr/">ACCESS CODE SCHOOL</a>
            <?php wp_nav_menu(array('theme_location' => 'Footer Menu')); ?>
            <div class="d-flex social">
                <a class="nav-link liensocial" href="#"><i class="fab fa-linkedin"></i></a>
                <a class="nav-link liensocial" href="#"><i class="fab fa-facebook-square"></i></a>
                <a class="nav-link liensocial" href="#"><i class="fab fa-twitter-square"></i></a>
            </div>
        
            <!-- <ul class="navbar-nav mr-auto">
            <li class="nav-item">
                <a class="nav-link" href="#">ACCUEIL</a>
            </li>
            <li class="nav-item">
                <a class="nav-link" href="#">ARTICLES</a>
            </li>
            <li class="nav-item">
                <a class="nav-link" href="#">PROJETS</a>
            </li>
            <li class="nav-item">
                <a class="nav-link" href="#">LA TEAM</a>
            </li>
            <li class="nav-item">
                <a class="nav-link" href="#">MENTIONS LEGALES</a>
            </li>
            <li class="nav-item">
                <a class="nav-link" href="#"><i class="fab fa-linkedin"></i></a>
                <a class="nav-link" href="#"><i class="fab fa-facebook-square"></i></a>
                <a class="nav-link" href="#"><i class="fab fa-twitter-square"></i></a>
            </li>
            </ul> -->
        </nav>
    </div>
</footer>